...
class EUser(User):
    ...
    account_number_validator = RegexValidator(
        regex=r"^\d{10}$", message="Enter a valid account Number"
    )
...
class FoodImage(Image):
    food_images = models.ImageField(
        upload_to="products/foods/%Y_%m_%d/",
...
class MeatImage(Image):
    meat_images = models.ImageField(
        upload_to="products/meats/%Y_%m_%d/",
        height_field="image_height",
        width_field="image_width",
    )
...
class Cart(models.Model):
    user = models.OneToOneField(Customer, on_delete=models.CASCADE, blank=True)
...
class OrderItem(models.Model):
    ...
    quantity = models.IntegerField()
    price = models.DecimalField(
        help_text="Price of all the quantities in naira", decimal_places=2, max_digits=7
    )


class Order(models.Model):
    customer = models.OneToOneField(Customer, on_delete=models.CASCADE, blank=True)
    status_code = models.ForeignKey("OrderStatusCode", on_delete=models.CASCADE)
...
class Shipment(models.Model):
    ...
    tracking_no = models.CharField(validators=[tracking_no_validator], max_length=20)
